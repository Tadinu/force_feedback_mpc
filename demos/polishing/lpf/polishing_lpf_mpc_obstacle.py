...
import mim_solvers
from force_feedback_dgh.mim_robots.robot_loader import load_bullet_wrapper
...
def solveOCP(q, v, tau, solver, nb_iter, target_reach, TASK_PHASE, target_force):
    ...
    if (TASK_PHASE == 3):
        for k in range(solver.problem.T + 1):
            # for k in range( 0, solver.problem.T+1, 1 ):  
            m[k].differential.costs.costs["translation"].active = True
            m[k].differential.costs.costs["translation"].cost.residual.reference = target_reach[k]
            m[k].differential.costs.costs["translation"].cost.activation.weights = np.array([1., 1., 0.])
            m[k].differential.costs.costs["translation"].weight = 200.
            m[k].differential.costs.costs['rotation'].active = True
            m[k].differential.costs.costs['rotation'].cost.residual.reference = pin.utils.rpyToMatrix(np.pi, 0, np.pi)
...
config_name = 'polishing_lpf_obstacle'
...
contact_surface_bulletId = simulator_utils.display_contact_surface(
    pin.SE3(np.eye(3), np.asarray(config['contactPosition'])), bullet_endeff_ids=robot_simulator.bullet_endeff_ids)
...
# !!!! CRITICAL PARAMETER !!!!
# Used to inerpolate (command sent to robot) between current & predicted torque
...
for k, m in enumerate(models):
    ...
    lpf_lb = -robot_simulator.pin_robot.model.effortLimit[lpfStateIds]
    lpf_ub = robot_simulator.pin_robot.model.effortLimit[lpfStateIds]
...
# if(PLOT_INIT):
#   ddp_handler = OCPDataHandlerLPF(solver.problem, n_lpf=n_lpf)
...
if (config['DISPLAY_EE']):
    ...
    for i in range(nb_points):
        ...
        pos = circle_point_WORLD(t, contact_placement_0, radius=RADIUS, omega=OMEGA,
...
for i in range(config['N_simu']):
    ...
    if (0 <= time_to_circle and time_to_circle % OCP_TO_CTRL_RATIO == 0):
        ...
        target_position[:, :2] = target_position_traj[
                                     time_to_circle:tf:OCP_TO_CTRL_RATIO, :2] + position_at_contact_switch[:2] - oPc[:2]
    # # # # # # # # #
    # # Solve OCP # #
    # # # # # # # # #
    ...
    if (i % int(sim_data.simu_freq / sim_data.plan_freq) == 0):
        ...
        x_filtered = antiAliasingFilter.step(nb_plan, i, sim_data.plan_freq, sim_data.simu_freq,
                                             sim_data.state_mea_SIMU)
        q = x_filtered[:nq]
    ...
    if (i % int(sim_data.simu_freq / sim_data.ctrl_freq) == 0):
        ...
        tau_mea_CTRL = antiAliasingFilter.step(nb_ctrl, i, sim_data.ctrl_freq, sim_data.simu_freq,
                                               sim_data.state_mea_SIMU[:, -n_lpf:])
        tau_mea_derivative_CTRL = antiAliasingFilter.step(nb_ctrl, i, sim_data.ctrl_freq, sim_data.simu_freq,
                                                          sim_data.tau_mea_derivative_SIMU[:, -n_lpf:])
        #  Select the desired torque as interpolation between current and prediction
        tau_des_CTRL = sim_data.y_curr[-n_lpf:] + EPSILON_INTERP * (sim_data.y_pred[-n_lpf:] - sim_data.y_curr[-n_lpf:])
        #  Optionally interpolate to the control frequency using Riccati gains
        if (config['RICCATI']):
            y_filtered = antiAliasingFilter.step(nb_ctrl, i, sim_data.ctrl_freq, sim_data.simu_freq,
                                                 sim_data.state_mea_SIMU)
        ...
        if (config['USE_LATERAL_FRICTION'] and 0 <= time_to_contact):
            Jac = pin.computeFrameJacobian(robot.model, robot.data, q, id_endeff, pin.LOCAL_WORLD_ALIGNED)[
                :3, lpfStateIds]
...
save_name = config_name + \
            ...
            '_Fp=' + str(sim_data.plan_freq / 1000) + '_Fc=' + str(sim_data.ctrl_freq / 1000) + '_Fs' + str(
    sim_data.simu_freq / 1000)
