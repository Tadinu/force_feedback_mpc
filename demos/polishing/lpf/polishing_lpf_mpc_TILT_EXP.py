...
from force_feedback_mpc.demos.polishing.polishing_lpf_mpc import solveOCP
...
def main(SAVE_DIR, TORQUE_TRACKING):
    ...
    config = load_yaml_file(
        '/home/sebastien/workspace_seb/src/force-feedback/demos/sanding/config/iiwa_LPF_sanding_MPC.yml')
    ...
    for n_seed in range(N_SEEDS):
        ...
        for n_exp in range(N_EXP):
            ...
            print("\n       ============================================================================= ")
            print("       Set TILT ANGLE to " + str(TILT_ANGLES_DEG[n_exp]) + " (" + str(n_exp + 1) + "/" + str(
                N_EXP) + ")")
            print("       ============================================================================= \n")
            #  # # # # # # # #
            ### OCP SETUP ###
            ...
            contact_surface_bulletId = simulator_utils.display_contact_surface(contact_placement,
            ...
            for i in range(sim_data.N_simu):
                ...
                if (0 <= time_to_circle and time_to_circle % OCP_TO_CTRL_RATIO == 0):
                    ...
                    target_position[:, :2] = target_position_traj[
                                                 time_to_circle:tf:OCP_TO_CTRL_RATIO, :2] + position_at_contact_switch[
                                                 :2] - oPc[:2]
                ...
                if (i % int(sim_data.simu_freq / sim_data.ctrl_freq) == 0):
                    ...
                    tau_mea_derivative_CTRL = antiAliasingFilter.step(nb_ctrl, i, sim_data.ctrl_freq,
                                                                      sim_data.simu_freq,
                                                                      sim_data.tau_mea_derivative_SIMU[:, -n_lpf:])
                    #  Select the desired torque as interpolation between current and prediction
                    tau_des_CTRL = sim_data.y_curr[-n_lpf:] + EPSILON_INTERP * (
                                sim_data.y_pred[-n_lpf:] - sim_data.y_curr[-n_lpf:])
                    ...
                    if (config['RICCATI']):
                        ...
                        Ktilde[:, -n_lpf:] += (1 - (1 - alpha) * sim_data.OCP_TO_PLAN_RATIO) * np.eye(
                            n_lpf)  # only for torques
                ...
                if (i >= T_CIRCLE):
                    ...
                    p_err.append(
                        np.abs(robot_simulator.pin_robot.data.oMf[id_endeff].translation[:2] - target_position[0][:2]))
