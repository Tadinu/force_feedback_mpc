...
def main(SAVE_DIR=None, TORQUE_TRACKING=False):
    #  # # # # # # # # # # # # # # # # # #
    ...
    softContactModel = DAMSoft1D(state,
                                 actuation,
                                 costModelSum,
                                 ...
                                 pinRefFrame=config['pinRefFrame'])

    #  Measure initial force in pybullet
    f0 = np.zeros(6)
    assert (softContactModel.nc == 1)
    assert (softContactModel.pinRef == pin.LOCAL_WORLD_ALIGNED.name)
