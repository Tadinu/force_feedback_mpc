...
def main(PLOT=False, DISPLAY=True):
    ...
    # # # # # # # # # # # # 
    #  Read config file
    config = path_utils.load_yaml_file(os.path.dirname(os.path.realpath(__file__)) + '/polishing_classical.yml')
    ...
    if (PLOT):
        ...
        _, _ = ocp_data_handler.plot_ocp_results(ocp_data, which_plots=config['WHICH_PLOTS'], markers=['.'],
                                                 colors=['b'], SHOW=True)
    ...
    if (DISPLAY):
        ...
        for i in range(N_h):
            ...
            if ('force' in config['WHICH_COSTS']):
                # Display wrench
                wrench = solver.problem.runningDatas[i].differential.multibody.contacts.contacts[
                    config['contacts'][0]['contactModelFrameName']].f.vector
