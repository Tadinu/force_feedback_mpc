...
def plot_ocp_solution(mpc):
    ...
    for time_idx in range(T):
        ...
        for fname in mpc.ee_frame_names:
            ...
            f.append(
                mpc.solver.problem.runningDatas[0].differential.multibody.contacts.contacts[ct_frame_name].f.linear)
    ...
    def createProblem(self):
        ...
        for t in range(self.HORIZON + 1):
            ...
            for i, frame_idx in enumerate(self.supportFeetIds):
                support_contact = crocoddyl.ContactModel3D(self.ccdyl_state, frame_idx, np.array([0., 0., 0.0]),
                                                           self.pinRef, self.nu, np.array([0., 10.]))
                self.contactModel.addContact(self.rmodel.frames[frame_idx].name + "_contact", support_contact)

                # Contact model for the EE
            arm_contact = crocoddyl.ContactModel3D(self.ccdyl_state, self.armEEId, self.armEEPos0,
                                                   pin.LOCAL_WORLD_ALIGNED, self.nu, np.array([0., 10.]))
            ...
            if t != self.HORIZON:
                ...
                contact_force_residual = crocoddyl.ResidualModelContactForce(self.ccdyl_state, self.armEEId,
                                                                             self.ef_des_force, 3, self.nu)
                contact_force_activation = crocoddyl.ActivationModelWeightedQuad(np.array([1., 1., 1.]))
                contact_force_track = crocoddyl.CostModelResidual(self.ccdyl_state, contact_force_activation,
            ...
            if (t != self.HORIZON):
                ...
                for frame_idx in self.supportFeetIds:
                    # Friction cones
                    name = self.rmodel.frames[frame_idx].name + "_contact"
                    residualFriction = friction_utils.ResidualFrictionCone(self.ccdyl_state, name, self.friction_mu,
                                                                           self.ccdyl_actuation.nu, normal='z')
                    constraintFriction = crocoddyl.ConstraintModelResidual(self.ccdyl_state, residualFriction,
                    ...
                    residualForce = crocoddyl.ResidualModelContactForce(self.ccdyl_state, frame_idx, pin.Force.Zero(),
                                                                        3, self.nu)
                    forceBoxConstraint = crocoddyl.ConstraintModelResidual(self.ccdyl_state, residualForce,
                ...
                residualFriction = friction_utils.ResidualFrictionCone(self.ccdyl_state, name, self.friction_mu,
                                                                       self.ccdyl_actuation.nu, normal='x')
                constraintFriction = crocoddyl.ConstraintModelResidual(self.ccdyl_state, residualFriction,
                                                                       np.array([0.]), np.array([np.inf]))
                # constraintModelManager.addConstraint(name + "friction", constraintFriction)
            ...
            dmodel = crocoddyl.DifferentialActionModelContactFwdDynamics(self.ccdyl_state, self.ccdyl_actuation,
                                                                         self.contactModel, costModel,
                                                                         constraintModelManager, 0., True)
