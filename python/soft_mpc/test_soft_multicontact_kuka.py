...
for i in range(N):
    ...
    if (CONTACT):
        softContactModelsStack = ViscoElasticContact3d_Multiple(state, actuation,
                                                                [ViscoElasticContact3D(state, actuation, frameId, oPc,
                                                                                       Kp, Kv,
    ...
    if (FORCE_COST):
        forceCostManager = ForceCostManager([ForceCost(state, frameId, np.array([0] * 3), 0., pin.LOCAL_WORLD_ALIGNED)],
    ...
    if (FORCE_C):
        ...
        forceConstraintManager = ForceConstraintManager([FrictionConeConstraint(frameId, MU),
                                                         ForceBoxConstraint(frameId, np.array([-np.inf] * 3),
                                                                            np.array([np.inf] * 3))],
                                                        softContactModelsStack)
    else:
    ...
    IAM = IAMSoftContactDynamics3D_Go2(DAM, dt=0.01, withCostResidual=True,
                                       forceConstraintManager=forceConstraintManager)
