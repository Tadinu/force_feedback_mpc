...
'''
from force_feedback_mpc.core_mpc_utils.misc_utils import CustomLogger, GLOBAL_LOG_LEVEL, GLOBAL_LOG_FORMAT
...
# Custom state model
class StateSoftContact1D(crocoddyl.StateAbstract):
    ...
    def Jintegrate(self, y, dy, Jfirst):
        ...
        Default values :
         ...
         op = crocoddyl.addto
        '''
        Jfirst[:self.nv, :self.nv] = pin.dIntegrate(self.pinocchio, y[:self.nq], dy[:self.nv],
                                                    pin.ARG0)  # , crocoddyl.addto)
...
class IAMSoftContactDynamics1D(crocoddyl.ActionModelAbstract):  # IntegratedActionModelAbstract
    def __init__(self, dam, dt=1e-3, withCostResidual=True):
        ...
        crocoddyl.ActionModelAbstract.__init__(self, crocoddyl.StateVector(dam.state.nq + dam.state.nv + 1), dam.nu,
                                               dam.costs.nr + 1)
        self.differential = dam
        # self.state = StateSoftContact1D(dam.pinocchio, 3)
        self.stateSoft = StateSoftContact1D(dam.pinocchio, 1)
        self.dt = dt
        self.withCostResidual = withCostResidual

    def createData(self):
        data = IADSoftContactDynamics1D(self)
        return data
...
class IADSoftContactDynamics1D(crocoddyl.ActionDataAbstract):  # IntegratedActionDataAbstract
    '''
...
class DAMSoftContactDynamics1D(crocoddyl.DifferentialActionModelAbstract):
    ...
    def __init__(self, stateMultibody, actuationModel, costModelSum, frameId, Kp=1e3, Kv=60, oPc=np.zeros(3),
                 pinRefFrame=pin.LOCAL, type=2):
    ...
    def calc(self, data, x, f, u):
        ...
        if (self.active_contact):
            ...
            fLOCAL = np.zeros(3);
            fLOCAL[self.mask] = f
            data.fext[self.parentId] = self.jMf.act(pin.Force(fLOCAL, np.zeros(3)))
            data.fext_copy = data.fext.copy()
            # Rotate if not f not in LOCAL
            if (self.pinRef != pin.LOCAL):
                data.fext[self.parentId] = self.jMf.act(pin.Force(oRf.T @ fLOCAL, np.zeros(3)))
            ...
            data.fout3d = -self.Kp * lv - self.Kv * la
            data.fout3d_copy = data.fout.copy()
            data.fout = data.fout3d[self.mask]
            data.fout_copy = data.fout
            # Rotate if not f not in LOCAL
            if (self.pinRef != pin.LOCAL):
                oa = pin.getFrameAcceleration(self.pinocchio, data.pinocchio, self.frameId,
                ...
                data.fout3d = -self.Kp * ov - self.Kv * oa
                data.fout = data.fout3d[self.mask]
                assert (np.linalg.norm(data.fout3d - oRf @ data.fout3d_copy) < 1e-3)
        else:
    ...
    def calcDiff(self, data, x, f, u):
        ...
        if (self.active_contact):
            ...
            aba_dq, aba_dv, aba_dtau = pin.computeABADerivatives(self.pinocchio, data.pinocchio, q, v,
            ...
            data.dABA_df = data.pinocchio.Minv @ lJ[:3].T @ self.pinocchio.frames[
                self.frameId].placement.rotation @ np.eye(3)
            ...
            _, a_dq, a_dv, a_da = pin.getFrameAccelerationDerivatives(self.pinocchio, data.pinocchio, self.frameId,
                                                                      pin.LOCAL)
            ...
            if (self.pinRef != pin.LOCAL):
                ...
                data.dfdt_dx[:, :self.state.nq] = oRf @ ldfdt_dx_copy[:, :self.state.nq] - pin.skew(
                    oRf @ data.fout_copy) @ oJ[3:]
...
class DADSoftContactDynamics(crocoddyl.DifferentialActionDataAbstract):
    ...
    def __init__(self, am):
        ...
        # Force model + derivative
        self.fout3d = np.zeros(3)
        self.fout3d_copy = np.zeros(3)
