...
if (TYPE == 'classical'):
    ...
    CONFIG_PATH = '/media/ducthan/376b23a1-5a02-4960-b3ca-24b2fcef8f891/10_IMPEDANCE_CONTROL/force_feedback_mpc/python/soft_mpc/Go2MPC_demo_soft.yml'
...
for i in range(N_SIMU):
    ...
    fric_res_l6 = MU * np.abs(measured_forces_dict['Link6'][i][0]) - np.sqrt(
        measured_forces_dict['Link6'][i][1] ** 2 + measured_forces_dict['Link6'][i][2] ** 2)
    fric_res_FL = MU * np.abs(measured_forces_dict['FL_FOOT'][i][2]) - np.sqrt(
        measured_forces_dict['FL_FOOT'][i][0] ** 2 + measured_forces_dict['FL_FOOT'][i][1] ** 2)
    fric_res_FR = MU * np.abs(measured_forces_dict['FR_FOOT'][i][2]) - np.sqrt(
        measured_forces_dict['FR_FOOT'][i][0] ** 2 + measured_forces_dict['FR_FOOT'][i][1] ** 2)
    fric_res_HL = MU * np.abs(measured_forces_dict['HL_FOOT'][i][2]) - np.sqrt(
        measured_forces_dict['HL_FOOT'][i][0] ** 2 + measured_forces_dict['HL_FOOT'][i][1] ** 2)
    fric_res_HR = MU * np.abs(measured_forces_dict['HR_FOOT'][i][2]) - np.sqrt(
        measured_forces_dict['HR_FOOT'][i][0] ** 2 + measured_forces_dict['HR_FOOT'][i][1] ** 2)
...
axs[0].plot(time_span, np.abs(measured_forces_dict['Link6'][:, 0]), linewidth=4, color='g', marker='o', alpha=0.5,
            label="Fx mea")
...
axs[1].plot(time_span, measured_forces_dict['Link6'][:, 1], linewidth=4, color='g', marker='o', alpha=0.5,
            label="Fy mea")
...
axs[2].plot(time_span, measured_forces_dict['Link6'][:, 2], linewidth=4, color='g', marker='o', alpha=0.5,
            label="Fz mea")
...
for i, fname in enumerate(ee_frame_names[:-1]):
    # x,y
    axs[0, i].plot(time_span, measured_forces_dict[fname][:, 0], linewidth=4, color='g', marker='o', alpha=0.5,
                   label="Fx measured")
    # axs[0, i].plot(time_span, predicted_forces_dict[fname][:,0], linewidth=4, color='b', marker='o', alpha=0.25, label="Fx predicted")
    axs[1, i].plot(time_span, measured_forces_dict[fname][:, 1], linewidth=4, color='g', marker='o', alpha=0.5,
                   label="Fy measured")
    ...
    axs[2, i].plot(time_span, measured_forces_dict[fname][:, 2], linewidth=4, color='g', marker='o', alpha=0.5,
                   label="Fz measured")
    # axs[2, i].plot(time_span, predicted_forces_dict[fname][:,2], linewidth=4, color='b', marker='o', alpha=0.25, label="Fz predicted")
    axs[2, i].plot(time_span, Fz_lb_mea, '--', linewidth=4, color='k', alpha=0.5,
...
axs[0].plot(time_span2, gap_norm + constraint_norm, linewidth=4, color='g', marker='o', alpha=0.5,
            label="Constraint norm")
# axs[0].set_ylim(0., 105)
axs[1].plot(time_span, np.abs(measured_forces_dict['Link6'][:, 0]), linewidth=4, color='g', marker='o', alpha=0.5,
            label="Measured force (Fx)")
axs[1].plot(time_span, np.abs(desired_forces[:, 0]), linewidth=4, color='k', marker='o', alpha=0.25,
