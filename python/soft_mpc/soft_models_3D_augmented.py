...
class IAMSoftContactDynamics3D(crocoddyl.ActionModelAbstract):  # IntegratedActionModelAbstract
    def __init__(self, dam, dt=1e-3, withCostResidual=True, frictionConeConstaints=[]):
        ...
        crocoddyl.ActionModelAbstract.__init__(self, force_feedback_mpc.StateSoftContact(dam.pinocchio, 3), dam.nu,
                                               dam.costs.nr + 3, ng, 0)
    ...
    def calcDiff(self, data, y, u=None):
        ...
        if (u is not None):
            ...
            data.Fx += self.stateSoft.Jintegrate(y, data.dx, crocoddyl.Jcomponent.first).tolist()[
                0]  # add identity to Fx = d(x+dx)/dx = d(q,v)/d(q,v)
...
class DAMSoftContactDynamics3D(crocoddyl.DifferentialActionModelAbstract):
    ...
    def __init__(self, stateMultibody, actuationModel, costModelSum, frameId, Kp=1e3, Kv=60, oPc=np.zeros(3),
                 pinRefFrame=pin.LOCAL, constraintModelManager=None):
        ...
        crocoddyl.DifferentialActionModelAbstract.__init__(self, stateMultibody, actuationModel.nu, costModelSum.nr, ng,
                                                           nh)
        self.Kp = Kp
        self.Kv = Kv
        ...
        if (u is not None):
            ...
            if (self.active_contact):
                ...
                if (self.pinRef != pin.LOCAL):
                    ...
                    ov = pin.getFrameVelocity(self.pinocchio, data.pinocchio, self.frameId,
                                              pin.LOCAL_WORLD_ALIGNED).linear
...
for i in range(N):
    ...
    # Constraints
    constraintModelManager = crocoddyl.ConstraintModelManager(state, actuation.nu)
    uBoxCstr = crocoddyl.ConstraintModelResidual(state, crocoddyl.ResidualModelControl(state, actuation.nu),
                                                 -robot.model.effortLimit, robot.model.effortLimit)
