...
from force_feedback_mpc.soft_mpc.soft_models_1D_augmented import IAMSoftContactDynamics1D as IAMSoft1D
...
class OptimalControlProblemSoftContactAugmented(OptimalControlProblemAbstract):
    ...
    def create_differential_action_model(self, state, actuation, costModelSum, softContactModel,
        ...
        dam.ref = softContactModel.pinRef
        dam.cost_ref = softContactModel.pinRef
        logger.warning("Set dynamics and cost reference frames to " + str(softContactModel.pinRef))
        return dam
    ...
    def finalize_running_model(self, state, actuation, runningModel, softContactModel, node_id):
        ...
        if ('frictionCone' in self.WHICH_CONSTRAINTS):
            ...
            residual = force_feedback_mpc.ResidualModelFrictionConeAugmented(state, fid, self.frictionCoefficient,
                                                                             actuation.nu)
    ...
    def success_log(self, softContactModel):
        ...
        logger.info("    SOFT CONTACT MODEL [ oPc=" + str(softContactModel.oPc) + \
                    ...
                    ', pinRef=' + str(softContactModel.pinRef) + ']')
    ...
    def initialize(self, y0, softContactModel):
        ...
        for i in range(self.N_h):
            ...
            if (self.nb_constraints == 0):
                ...
                dam = self.create_differential_action_model(state, actuation, costModelSum, softContactModel,
                                                            constraintModelManager)
            runningModels.append(IAMSoft1D(dam, self.dt))
            # import pdb; pdb.set_trace()
        ...
        if (self.nb_constraints == 0):
            ...
            dam_t = self.create_differential_action_model(state, actuation, costModelSum_t, softContactModel,
                                                          constraintModelManager)
        terminalModel = IAMSoft1D(dam_t, 0.)
