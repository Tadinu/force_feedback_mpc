...
def getForceSensor(model, data, site_name):
    ...
    # print("SENSOR NAME = \n", sensor_name)
    force_site_to_sensor_idx = {'FL_force_site': 10, 'FR_force_site': 13, 'RL_force_site': 16, 'RR_force_site': 19,
                                'EF_force_site': 22}
...
class Go2MPCSoft:
    def __init__(self, HORIZON=250, friction_mu=0.75, dt=0.01, USE_MUJOCO=True):
        ...
        if (self.USE_MUJOCO):
            ...
            self.assets_path = '/media/ducthan/376b23a1-5a02-4960-b3ca-24b2fcef8f891/10_IMPEDANCE_CONTROL/Go2Py/Go2Py/assets/'
        ...
        if (self.USE_MUJOCO):
            ...
            self.mpc_to_unitree_idx = np.zeros(18).astype(
                np.int32)  # mpc_state[mpc_to_unitree_idx] -> state/command in unitree order
            self.unitree_to_mpc_idx = np.zeros(18).astype(
                np.int32)  # unitree_state[unitree_to_mpc] -> state/command in mpc order
    ...
    def createProblem(self):
        ...
        for t in range(self.HORIZON + 1):
            ...
            # else:
            #     costModel.addCost("comTrack", com_track, com_weight*self.dt)
            ...
            lf_contact = ViscoElasticContact3D(self.ccdyl_state, self.ccdyl_actuation, self.lfFootId,
                                               self.rdata.oMf[self.lfFootId].translation.copy(), self.Kp, self.Kv,
                                               self.pinRef)
            rf_contact = ViscoElasticContact3D(self.ccdyl_state, self.ccdyl_actuation, self.rfFootId,
                                               self.rdata.oMf[self.rfFootId].translation.copy(), self.Kp, self.Kv,
                                               self.pinRef)
            lh_contact = ViscoElasticContact3D(self.ccdyl_state, self.ccdyl_actuation, self.lhFootId,
                                               self.rdata.oMf[self.lhFootId].translation.copy(), self.Kp, self.Kv,
                                               self.pinRef)
            rh_contact = ViscoElasticContact3D(self.ccdyl_state, self.ccdyl_actuation, self.rhFootId,
                                               self.rdata.oMf[self.rhFootId].translation.copy(), self.Kp, self.Kv,
                                               self.pinRef)
            ef_contact = ViscoElasticContact3D(self.ccdyl_state, self.ccdyl_actuation, self.armEEId, self.oPc_ee,
                                               self.Kp, self.Kv, self.pinRef)
            # Stack models
            softContactModelsStack = ViscoElasticContact3d_Multiple(self.ccdyl_state, self.ccdyl_actuation,
                                                                    [lf_contact, rf_contact, lh_contact, rh_contact,
                                                                     ef_contact])
            ...
            # Fz_ref_foot = 50
            if t != self.HORIZON:
                forceCostEE = ForceCost(self.ccdyl_state, self.armEEId, np.array([-self.Fx_ref_ee, 0., 0.]), f_weight,
                ...
                forceCostEE = ForceCost(self.ccdyl_state, self.armEEId, np.array([-self.Fx_ref_ee, 0., 0.]),
                                        f_weight * self.dt, pin.LOCAL_WORLD_ALIGNED)
            ...
            iam = IAMSoftContactDynamics3D_Go2(dam, dt=self.dt, withCostResidual=True,
                                               forceConstraintManager=forceConstraintManager)
    ...
    def setup_meshcat_force_vizualisation(self, viz, MU):
        ...
        arrow_ee = meshcat_utils.Arrow(viz.viewer, "force_ee", location=[0, 0, 0], vector=[0, 0, 0.01],
                                       length_scale=0.01)
