...
def load_bullet_wrapper(robot_name, locked_joints=None):
    ...
    try:
        if (robot_name == 'iiwa'):
            from force_feedback_dgh.mim_robots.robots.kuka.pinbullet.iiwaWrapper import IiwaRobot
            return IiwaRobot(MiM_Robots["iiwa"], locked_joints)
        elif (robot_name == 'iiwa_convex'):
            from force_feedback_dgh.mim_robots.robots.kuka.pinbullet.iiwaWrapper import IiwaRobot
            return IiwaRobot(MiM_Robots["iiwa_convex"], locked_joints)
            ...
            from force_feedback_dgh.mim_robots.robots.teststand.pinbullet.teststand_wrapper import TeststandRobot
            robot = TeststandRobot(MiM_Robots["teststand"])
        elif (robot_name == 'solo12'):
            from force_feedback_dgh.mim_robots.robots.solo12.pinbullet.solo12wrapper import Solo12Robot
    ...
    try:
        ...
        try:
            ...
            if (locked_joints is None):
                ...
                reduced_model, [visual_model, collision_model] = pin.buildReducedModel(full_robot.model,
                                                                                       [full_robot.visual_model,
                                                                                        full_robot.collision_model],
                                                                                       locked_joints_ids,
                                                                                       np.zeros(RobotInfo.nq))
                return pin.robot_wrapper.RobotWrapper(reduced_model, collision_model, visual_model)
        except Exception as e:
            print("ERROR in URDF:", e)
    except:
        try:
